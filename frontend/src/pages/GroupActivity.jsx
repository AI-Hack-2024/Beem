  ...
  return (
    <div>
      ...
      <div className='flex flex-row gap-10 justify-center items-center mt-10'>
        <p className={`font-medium ${filter === 'balances' ? 'underline' : ''}`} onClick={() => setFilter('balances')}>
          Balances
        </p>
        <p className={`font-medium ${filter === 'expenses' ? 'underline' : ''}`} onClick={() => setFilter('expenses')}>
          Expenses
        </p>
        <p className={`font-medium ${filter === 'activity' ? 'underline' : ''}`} onClick={() => setFilter('activity')}>
          Activity
        </p>
        <p className={`font-medium ${filter === 'memories' ? 'underline' : ''}`} onClick={() => setFilter('memories')}>
          Memories
        </p>
      </div>
      <div className="mt-6">
